...
export interface PullRequestContent { title: string; body: string }
...
export class CodexContentGenerator implements DevelopmentContentGenerator {
  constructor(
...
${JSON.stringify({ number: issue.number, title: issue.title, url: issue.url })}`);
  ...
  private execute(prompt: string): string {
