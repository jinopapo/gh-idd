...
export const closeCommand: CommandDefinition = {
  ...
  async execute(context) {
    ...
    if (result.next) lines.push(`Started Issue #${result.next.issue.number}: ${result.next.issue.title}`, `Branch: ${result.next.branch}`);
    else lines.push("No newly unblocked downstream Issue.");
