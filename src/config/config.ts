...
export function parseToml(text: string): ConfigObject {
  ...
  text.split(/\r?\n/).forEach((original, index) => {
    ...
    if (header?.[1]) {
      ...
      for (const key of header[1].split(".")) {
        ...
        if (!next || typeof next !== "object" || Array.isArray(next)) section[key] = {};
        section = section[key] as ConfigObject;
      }
      return;
    }
    const assignment = line.match(/^([A-Za-z0-9_-]+)\s*=\s*(.+)$/);
    if (!assignment?.[1] || !assignment[2]) throw new UserError(`Invalid TOML on line ${index + 1}`);
    section[assignment[1]] = parseValue(assignment[2], index + 1);
  });
  return result;
}

export function stringifyToml(config: ConfigObject): string {
  const lines: string[] = [];
  const emit = (object: ConfigObject, prefix: string[] = []): void => {
    ...
    if (prefix.length && scalars.length) lines.push(`[${prefix.join(".")}]`);
    for (const [key, value] of scalars) lines.push(`${key} = ${JSON.stringify(value)}`);
    if (prefix.length && scalars.length && groups.length) lines.push("");
    groups.forEach(([key, value], index) => {
      emit(value as ConfigObject, [...prefix, key]);
...
export function deepMerge(...objects: ConfigObject[]): ConfigObject {
  const target: ConfigObject = {};
  for (const object of objects) {
    for (const [key, value] of Object.entries(object)) {
      const previous = target[key];
      target[key] = value && typeof value === "object" && !Array.isArray(value)
        ? deepMerge(previous && typeof previous === "object" && !Array.isArray(previous) ? previous as ConfigObject : {}, value as ConfigObject)
        : value;
    }
  }
  return target;
...
function readConfig(file: string): ConfigObject {
  return fs.existsSync(file) ? parseToml(fs.readFileSync(file, "utf8")) : {};
}

export function configPaths(repoRoot: string, env: NodeJS.ProcessEnv = process.env): { global: string; local: string } {
...
export function loadConfig(repoRoot: string, overrides: ConfigObject = {}, env: NodeJS.ProcessEnv = process.env): AppConfig {
  const paths = configPaths(repoRoot, env);
  return deepMerge(DEFAULT_CONFIG as unknown as ConfigObject, readConfig(paths.global), readConfig(paths.local), overrides) as unknown as AppConfig;
}

export function saveLocalProject(repoRoot: string, owner: string, number: number): string {
...
export function requireProject(config: AppConfig): ProjectConfig {
  if (!config.project.owner || !config.project.number) throw new UserError("No project configured.\n\nRun:\n\n  gh idd set-project");
  return config.project as ProjectConfig;
}
