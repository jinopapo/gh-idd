...
const TYPES = new Map<string, RegExp>([
...
export function issueType(title: string): string {
  for (const [type, pattern] of TYPES) if (pattern.test(title)) return type;
  return "feat";
}
export function slugify(title: string): string {
  return title.normalize("NFKD").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 60).replace(/-$/g, "") || "issue";
}
export function branchName(issue: Pick<Issue, "number" | "title">, format: string, ai = ""): string {
