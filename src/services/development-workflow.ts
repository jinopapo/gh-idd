...
export interface StartResult { issue: Issue; branch: string }
export interface PrResult extends StartResult { pr: PullRequest }
export interface CloseResult { issue: Issue; next: StartResult | null }
export type CurrentIssueResult = StartResult;
export interface StatusResult { issue: Issue; branch: string; project: ProjectConfig; item: ProjectItem | null; pr: PullRequest | null }
...
export class DevelopmentWorkflow {
  constructor(
    ...
    private readonly contentGenerator?: DevelopmentContentGenerator,
  ) {}

  start(issueNumber: string | number, options: { force?: boolean } = {}): StartResult {
    const issue = this.github.issue(issueNumber);
    if (issue.state !== "OPEN") throw new UserError(`Issue #${issue.number} is ${issue.state.toLowerCase()}.`);
    ...
    if (this.config.workflow.assign_on_start) this.github.assign(issue);
    if (project) this.github.updateProject(issue, project, project.status.progress);
    return { issue, branch };
  }

  async nextIssue(options: { force?: boolean } = {}): Promise<StartResult> {
    const current = this.github.issueForBranch(this.git.currentBranch());
    const next = await this.startNextIssue(current, options);
    if (!next) throw new UserError(`Issue #${current.number} did not unblock any issues.`);
    return next;
  }

  async closeIssue(options: { force?: boolean } = {}): Promise<CloseResult> {
    const issue = this.github.issueForBranch(this.git.currentBranch());
    if (issue.state !== "OPEN") throw new UserError(`Issue #${issue.number} is already closed.`);
    this.github.closeIssue(issue);
    return { issue, next: await this.startNextIssue(issue, options) };
  }

  private async startNextIssue(current: Issue, options: { force?: boolean }): Promise<StartResult | null> {
    ...
    if (!selected) throw new UserError("No next Issue selected.");
  ...
  createPr(): PrResult {
    ...
    if (project) this.github.updateProject(issue, project, project.status.review);
  ...
  private generator(): DevelopmentContentGenerator {
    if (!this.contentGenerator) throw new UserError("The configuration uses an AI placeholder, but no Codex generator is available.");
    return this.contentGenerator;
  }
}

function renderTemplate(template: string, values: Record<string, string>): string {
  return template.replace(/\{(issue|issue_title|branch|ai_title|ai_body)\}/g, (_match, key: string) => values[key] || "");
}

export function formatStatus({ issue, branch, project, item, pr }: StatusResult): string {
  const status = item?.status || item?.fieldValues?.find((field) => field.field?.name === (project.status_field || "Status"))?.name || "not in project";
  const issueLines = (issues: Issue["blockedBy"]): string[] => issues.length ? issues.map((entry) => `  #${entry.number} ${entry.title}`) : ["  none"];
  return [
...
export function formatCurrentIssue({ issue }: CurrentIssueResult): string {
